...
container.setAttribute(
  "style",
  "margin: auto; width: 30%; background-color: aliceblue;"
);
body.append(container);

const pokemonSprites = Object.keys(pokemon.sprites);
pokemonSprites.forEach((key) => {
  const spriteImages = `${pokemon.sprites[key]}`;

  if (spriteImages !== "null") {
    const image = document.createElement("img");
    image.src = `${pokemon.sprites[key]}`;
